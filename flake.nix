{
  ...
  outputs = { self, stable-nixpkgs, unstable-nixpkgs, home-manager, sops-nix
    , ... }@inputs:
    ...
    in {
      ...
      nixosConfigurations = {
        slax = stable-nixpkgs.lib.nixosSystem {
          ...
          modules =
            [ ./hosts/slax/configuration.nix sops-nix.nixosModules.sops ];
        };
        brix = stable-nixpkgs.lib.nixosSystem {
          specialArgs = { inherit inputs outputs; };
          # > Our main nixos configuration file <
          modules =
            [ ./hosts/brix/configuration.nix sops-nix.nixosModules.sops ];
        };
      };
